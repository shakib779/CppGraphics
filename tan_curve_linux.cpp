...
int main(){
   ...
   // cout << maxy << newl;
    ...
    setcolor(11);
    outtextxy(100, 100, "tangent curve,  y = tan(x)");


    setcolor(10);
    for(x = 0, angle = 0, prevx = maxx/2, prevy = maxy/2; x <= maxx/2; x++, angle++) {
        if(angle%180 > 67) continue;
    ...
    for(x = -1, angle = -1,  prevx = maxx/2, prevy = maxy/2; x > -(maxx/2); x--, angle--) {
        if(angle%180 < -67) continue;
        y = 100*tan(degtorad(angle));
        y = maxy/2 - y;
        putpixel(maxx/2 + x, y, 12);
